...
import { CommandsProps, TextMessage, AttachmentObject } from "../index.js";
import loadAttachment from "./loadAttachment.js";
import { isAttachmentObject } from "./parser.js";

type CommandParserReturnTypes = { status: true, data: CommandsProps } | { status: false, data: null };
...
export default function commandParser(message: TextMessage, prefix: string, api: IFCAU_API): CommandParserReturnTypes {
    if (
        ...
        const commandArgs = message.body.slice(prefix.length).trim().split(/ +/g);
