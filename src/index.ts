...
> extends infer R
    ? Extract<
      ...
      > extends infer S
        ...
        : never
    : never;
export type ReactionMessage = Pick<
    IFCAU_ListenMessage,
    Extract<keyof IFCAU_ListenMessage, "type">
> extends infer R
    ? Extract<IFCAU_ListenMessage, { type: "message_reaction" }> extends infer S
        ...
        : never
    : never;
export type UnsendMessage = Pick<
    IFCAU_ListenMessage,
    Extract<keyof IFCAU_ListenMessage, "type">
> extends infer R
    ? Extract<IFCAU_ListenMessage, { type: "message_unsend" }> extends infer S
        ? R & S
        : never
    : never;
export type OtherMessage = Exclude<
    IFCAU_ListenMessage,
    EventMessage | TextMessage | ReactionMessage | UnsendMessage
>;
...
export declare interface Client {
    on(
        event: "logged",
        listener: (api: IFCAU_API, userID: string) => void
    ): this;
    on(event: "error", listener: (err: Error) => void): this;
    on(
        event: "message",
        listener: (message: TextMessageExtended) => void
    ): this;
    on(event: "command", listener: (props: CommandsProps) => void): this;
    on(
        event: "reaction",
        listener: (message: ReactionMessageExtended) => void
    ): this;
    on(
        event: "unsend",
        listener: (message: UnsendMessageExtended) => void
    ): this;
    ...
    listen() {
        ...
        return api.listenMqtt((err, message) => {
            ...
            if (message.type === "message_reaction") {
                return this.emit(
                    "reaction",
                    reactionMessageParser(message, api)
                );
            }
